...
class ParserVisitor(MainVisitor):
    ...
    def visitStart(self, ctx:MainParser.StartContext):
        if ctx.init() is not None:
            self.output.write("_init = {}\n\n".format(self.visit(ctx.init())))
        for rule in ctx.parsingRule():
            nt, rules, attr = self.visit(rule)
            self.first[nt] = {}
            self.follow[nt] = set()
            self.data[nt] = set(rules)
            self.attr[nt] = None if attr is None else ParserVisitor.brush(attr.getText())
        for nt in self.data:
            ...
            print(str(self.links[nt]))
            print(str(self.data[nt]))
            print(str(self.attr[nt]))
        self.build()
        for nt in self.data:
            self.output.write("class {}(nonterm):\n    def __init__(self):\n        self.__attr__ = {{}}\n\n".format(nt))
        for nt in self.data:
            self.output.write("{0}.rules = {1}\n{0}.first = {2}\n{0}.follow = {3}\n{0}.links = {4}\n{0}.rule = {5}\n\n".format(nt, self.wlist(self.data[nt]), self.wdict(self.first[nt]), self.wset(self.follow[nt]), self.links[nt], self.attr[nt]))


    def visitInit(self, ctx:MainParser.InitContext):
        return self.brush(ctx.CODE().getText())
